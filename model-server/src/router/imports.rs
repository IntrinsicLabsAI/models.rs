pub mod types {
    ...
    use crate::import::types::{ImportJobId, ImportJobStatus};
...
pub mod endpoints {
    ...
    use crate::{
        import::types::{ImportJob, ImportJobId, ImportJobStatus},
        router::models::types::Locator,
        state::AppState,
    };
    ...
    pub async fn import_job_status_all(
        State(app_state): State<AppState>,
    ) -> Result<Json<types::GetAllJobStatusResponse>, StatusCode> {
        ...
        Ok(Json(types::GetAllJobStatusResponse { import_jobs }))
    }
}
